    ...
    <div className="flex h-full min-h-0 flex-col bg-ink-950">
      <div className="flex items-center justify-between border-b border-ink-800 px-5 py-3">
        <div>
          ...
          <div className="mt-0.5 font-serif text-lg text-ink-100">
            {ws.data?.name ?? '—'}
            <span className="ml-2 font-mono text-[10px] text-ink-500">{ws.data?.path}</span>
          </div>
        </div>
        <button
          onClick={async () => {
          ...
          className="rounded border border-ink-700 bg-ink-900 px-3 py-1.5 font-mono text-[11px] uppercase tracking-widest2 text-amber hover:border-amber"
        >
          + new file
        </button>
      </div>
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="flex w-64 shrink-0 flex-col border-r border-ink-800 bg-ink-900/30">
          <div className="border-b border-ink-800 px-3 py-2 font-mono text-[10px] uppercase tracking-widest2 text-ink-500">
            files
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto py-2">
            {tree.isLoading && <div className="px-3 text-[12px] text-ink-400">…</div>}
            {tree.data && (
              <FileTree root={tree.data} activePath={activePath} onOpen={setActivePath} />
            )}
            {tree.data && (tree.data.children?.length ?? 0) === 0 && (
              <div className="px-3 py-4 font-mono text-[11px] text-ink-500">
                empty workspace.
                <br />
                create a file →
              </div>
...
function Empty({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex h-full items-center justify-center p-10 text-center">
      <div className="max-w-md rounded border border-dashed border-ink-700 bg-ink-900/30 px-8 py-10 font-mono text-[12px] text-ink-400">
        {children}
      </div>
    </div>
  );
